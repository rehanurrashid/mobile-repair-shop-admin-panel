...
<head>
    ...
    <title>Users | Welcome to keshavinfotech</title>
        ...
        <div id="content">
            ...
            <div class="container">
                ...
                <div class="page-header">
                    ...
                    <div class="col-md-9">
                        <div class="page-title">
                        <h4 id="message" class="text-success message hide">Users are Deleted Successfully!</h4>

                    <?php if(isset($_GET['message'])): $message =$_GET['message']; ?>
                    <?php    if($message=="true"){ ?>
                            <h4 class="text-success message">User Inserted Successfully!</h4>
                    <?php  }elseif($message=="false"){ ?>
                            <h4 class="text-danger message">User Failed to Insert. Please try Again!</h4>
                    <?php }
                    else if($message=="exists"){ ?>
                        <h4 class="text-danger message">User Already Exists!</h4>
                    <?php } elseif($message=="deletesuccessfully"){ ?>
                        <h4 class="text-success message">User Deleted Successfully!</h4>
                    <?php } elseif($message=="notdeleted"){ ?>
                        <h4 class="text-danger message">User Unable to Delete!</h4>
                    <?php } elseif($message=="editsucessfully"){ ?>
                        <h4 class="text-success message">User Updated Successfully!</h4>
                    <?php } elseif($message=="editunsucessfull"){ ?>
                        <h4 class="text-danger message">User Unable to Update!</h4>
                    <?php }?>
                    ...
                    <div class="col-md-12">
                        <div class="portlet box blue">
                            <div class="portlet-title">
                                <div class="caption"><i class="fa fa-table"></i>Users</div>
                            </div>
                            <div class="portlet-body">
                                <div class="tableFixHead">
<!--                                     <form name="frmNewsList" method="post" action="#"> -->
                                        <table class="table table-bordered table-hover " id="manageUsers" style="width: 100%">
                                            <thead>
                                              <tr>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
          App.init();
          manageUsers = $("#manageUsers").DataTable({
            'ajax': 'user/fetch.php',
            'orders': []
          });
...
<script type="text/javascript">
    function confirmDelete(uid){
        ...
        if(message==true){
            window.location.href = "User/delete.php?uid="+uid+"&token=999";
        }
        else{
            return false;
        }
    ...
    function confirmDeleteAll(){
        var message = confirm("Are your sure you want to delete all Selected Users?");
        if(message==true){
            var uid = [];
        $("input[name = 'case']:checked").each(function(){
            uid.push($(this).attr('id'));
            $.get(`user/delete.php`,{uid:uid},function(_data){
