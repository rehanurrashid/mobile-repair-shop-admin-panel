...
<head>
    ...
    <title>Devices | Welcome to keshavinfotech</title>
        ...
        <div id="content">
            ...
            <div class="container">
                ...
                <div class="page-header">
                    ...
                    <div class="col-md-9">
                        <div class="page-title">
                        <h4 id="message" class="text-success message hide">Devices are Deleted Successfully!</h4>

                    <?php if(isset($_GET['message'])): $message =$_GET['message']; ?>
                    <?php    if($message=="true"){ ?>
                            <h4 class="text-success message">Device Inserted Successfully!</h4>
                    <?php  }elseif($message=="false"){ ?>
                            <h4 class="text-danger message">Device Failed to Insert. Please try Again!</h4>
                    <?php }
                    else if($message=="exists"){ ?>
                        <h4 class="text-danger message">Device Already Exists!</h4>
                    <?php } elseif($message=="deletesuccessfully"){ ?>
                        <h4 class="text-success message">Device Deleted Successfully!</h4>
                    <?php } elseif($message=="notdeleted"){ ?>
                        <h4 class="text-danger message">Device Unable to Delete!</h4>
                    <?php } elseif($message=="editsucessfully"){ ?>
                        <h4 class="text-success message">Device Updated Successfully!</h4>
                    <?php } elseif($message=="editunsucessfull"){ ?>
                        <h4 class="text-danger message">Device Unable to Update!</h4>
                    <?php }?>
                    ...
                    <div class="col-md-12">
                        <div class="portlet box blue">
                            <div class="portlet-title">
                                <div class="caption"><i class="fa fa-table"></i>Devices</div>
                            </div>
                            <div class="portlet-body">
                                <div class="tableFixHead">
<!--                                     <form name="frmNewsList" method="post" action="#"> -->
                                        <table class="table table-bordered table-hover " id="manageDevices" style="width: 100%" >
                                            <thead>
                                              <tr>
                                        ...
                                        <div class="row">
                                            <div class="col-md-6">
                                                ...
                                                <input style="margin-right:7px;" type="button" name="button2" id="button2" value="Add New" onclick="location.href='add_device.php?mode=add'" class="btn green pull-left" />
                                                &nbsp;
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        App.init();
          manageDevices = $("#manageDevices").DataTable({
            'ajax': 'device/fetch.php',
            'orders': []
          });
...
<script type="text/javascript">
    function confirmDelete(did){
        ...
        if(message==true){
            window.location.href = "Device/delete.php?did="+did+"&token=999";
        }
        else{
            return false;
        }
    ...
    function confirmDeleteAll(){
        var message = confirm("Are your sure you want to delete all Selected Devices?");
        if(message==true){
            var did = [];
        $("input[name = 'case']:checked").each(function(){
            did.push($(this).attr('id'));
            $.get(`device/delete.php`,{did:did},function(_data){
            let data=JSON.parse(_data);
            if(data.response== true){
                $('#message').removeClass('hide').addClass('show').fadeOut(3000);
                var table = $('#manageDevices').DataTable();
                table.ajax.reload();
            }
            else{
