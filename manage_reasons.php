        ...
        <div id="content">
            ...
            <div class="container">
                ...
                <div class="page-header">
                    ...
                    <div class="col-md-9">
                        <div class="page-title">
                        <h4 id="message" class="text-success message hide">Reasons are Deleted Successfully!</h4>
                    ...
                    <?php } elseif($message=="notdeleted"){ ?>
                        <h4 class="text-danger message">Reason Unable to Delete!</h4>
                    <?php } elseif($message=="editsucessfully"){ ?>
                        <h4 class="text-success message">Reason Updated Successfully!</h4>
                    <?php } elseif($message=="editunsucessfull"){ ?>
                        <h4 class="text-danger message">Reason Unable to Update!</h4>
                    <?php }?>
...
<!--                                     <form name="frmNewsList" method="post" action="#"> -->
                                    <div class="table-responsive">
                                        <table class="table table-bordered table-hover" id="manageReasons" style="width: 100%" >
                                            <thead>
                                              <tr>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        App.init();
          manageReasons = $("#manageReasons").DataTable({
            'ajax': 'reason/fetch.php',
            'orders': []
          });
...
<script type="text/javascript">
    function confirmDelete(rid){
        ...
        if(message==true){
            window.location.href = "reason/delete.php?rid="+rid+"&token=999";
        }
        else{
            return false;
        }
    ...
    function confirmDeleteAll(){
        var message = confirm("Are your sure you want to delete all Selected Reasons?");
        if(message==true){
            var rid = [];
        $("input[name = 'case']:checked").each(function(){
            rid.push($(this).attr('id'));
            $.get(`reason/delete.php`,{rid:rid},function(_data){
            let data=JSON.parse(_data);
            if(data.response== true){
                $('#message').removeClass('hide').addClass('show').fadeOut(3000);
                var table = $('#manageReasons').DataTable();
                table.ajax.reload();
            }
            else{
